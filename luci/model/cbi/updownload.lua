...
ful = SimpleForm("upload", translate("Upload"), translate("Upload your files from local disk.(Modified by mingyue.)"))
ful.reset = false
ful.submit = false

sul = ful:section(SimpleSection, "", translate("Upload file to '/tmp/upload/'"))
fu = sul:option(FileUpload, "")
fu.template = "cbi/file_transfer_upload"
um = sul:option(DummyValue, "", nil)
um.template = "cbi/file_transfer_dvalue"

fdl = SimpleForm("download", translate("Download"), translate("Download files from devices."))
fdl.reset = false
fdl.submit = false
sdl = fdl:section(SimpleSection, "", translate("Download file"))
fd = sdl:option(FileUpload, "")
fd.template = "cbi/file_transfer_download"
dm = sdl:option(DummyValue, "", nil)
dm.template = "cbi/file_transfer_dvalue"
...
form = SimpleForm("filelist", translate("Upload file list"), translate("Install ipk here if you upload ipk files.If you upload bin file,you can upgrade system.Make sure your bin is obsolutely right."))
form.reset = false
form.submit = false
...
btnis.template = "cbi/file_transfer_button"
...
btnis = tb:option(Button, "upgrade", translate("Flash image..."))
btnis.template = "cbi/file_transfer_button" 
btnis.render = function(self, section, scope) 
	if not inits[section] then return false end 
	if IsBINFile(inits[section].name) then
		scope.display = ""
	else
		scope.display = "none"
	end 
...
btnis.write = function(self, section) 
	local r = luci.sys.exec(string.format('mtd -r write "/tmp/upload/%s" firmware', inits[section].name))
	form.description = string.format('<span style="color: red">%s</span>', r) 
end 
