...
local edge2 =(luci.sys.call("pidof edge2 > /dev/null") == 0)
local supernode2 =(luci.sys.call("pidof supernode2 > /dev/null") == 0)
if edge2 or supernode2 then	
	m = Map("n2n_v2", translate("N2N VPN(V2)"), translate("N2Nv2 is running"))
else
	m = Map("n2n_v2", translate("N2N VPN(V2)"), translate("N2N_v2 is not running"))
end
       

s = m:section(TypedSection, "edge", translate("N2N Edge Settings") ,translatef("n2n is a layer-two peer-to-peer virtual private network (VPN) which allows users to exploit features typical of P2P applications at network instead of application level."))
s.anonymous = true
